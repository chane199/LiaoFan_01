...
public class KindArrayAdapter extends BaseAdapter
{
		...
		ViewGroup parent)
	{
		...
		linear.setLayoutParams(new RelativeLayout.LayoutParams(LayoutParams.FILL_PARENT,LayoutParams.WRAP_CONTENT));
		...
		try
		{
			...
			//获取用户ID
			String category1=((JSONObject)getItem(position)).getString("category1");
			// 设置TextView所显示的内容
			tv.setText(kindName);
			if("edu".equals(category1))
			{
				...
				tvPost.setText(Html.fromHtml("<font color='#78b6fb'>"+category1+"</font>"));
