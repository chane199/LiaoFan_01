...
public class HomeActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
     ...
     /**      
      *   这一部分是将获得的字符行进行整理，获得一个字符串数组  
      *     int length=jsonArray.length();//长度
            List<String> args = new ArrayList<String>();
            for(int i=0;i<length;i++){
            	String kindDesc = ((JSONObject)getItem(i)).getString("title");
   				args.add(kindDesc);
            }
			
			ArrayAdapter<String> adapter=new ArrayAdapter<String>(this, R.layout.array_item,args);
